...
void HttpServerAdvancedSession::get_preload_param() {
    ...
    if (uri_parser.get_port().empty()) {
        preload_url_ = URIParser::build_url((is_https ? "https" : "http"), uri_parser.get_host(), uri_parser.get_path(), user_query);
    } else {
        preload_url_ = URIParser::build_url((is_https ? "https" : "http"), uri_parser.get_host(), (uint16_t)proxy::string_to_int(uri_parser.get_port()), uri_parser.get_path(), user_query);
    }
...
void HttpServerAdvancedSession::handle_head_request() {
    ...
    boost::beast::http::response_serializer<boost::beast::http::empty_body> response_serializer{response};
